...
def pivoted_cholesky_from_columns(
    ...
    *,
    ...
    while rank < cap:
        ...
        if xp is not np:
            transfer_counter.record_d2h(
                int(device_pivot.nbytes) + int(device_max.nbytes), count=2
            )
            host_scalar_reads += 2
        ...
        if xp is not np:
            transfer_counter.record_d2h(int(finite_column.nbytes))
            host_scalar_reads += 1
        ...
        if xp is not np:
            transfer_counter.record_d2h(int(corrected_pivot.nbytes))
            host_scalar_reads += 1
        ...
        if xp is not np:
            transfer_counter.record_d2h(int(minimum_residual.nbytes))
            host_scalar_reads += 1
    ...
    if rank == cap:
        device_max = xp.max(residual)
        if xp is not np:
            transfer_counter.record_d2h(int(device_max.nbytes))
            host_scalar_reads += 1
        residual_max = float(device_max.item())
    ...
    *,
    ...
    while rank < cap:
        ...
        if xp is np:
            ...
            transfer_counter.record_d2h(int(candidates_device.nbytes))
            host_scalar_reads += 1
        ...
        if xp is not np:
            transfer_counter.record_d2h(int(finite_batch.nbytes))
            host_scalar_reads += 1
        ...
        for offset, raw_pivot in enumerate(candidates):
            ...
            if xp is not np:
                transfer_counter.record_d2h(int(pivot_residual.nbytes))
                host_scalar_reads += 1
