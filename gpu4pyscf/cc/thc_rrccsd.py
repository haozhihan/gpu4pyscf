...
class THCRRCCSD(RRCCSD):
    ...
    endpoint. It evaluates and back-projects paper Algorithms 1--3 as an
    equation audit, but returns the unaltered complete RR equation. Lower THC
    ranks fail closed because those paper terms cross the coarse groups in the
    current RR residual builder and cannot yet be subtracted with a proven
    complement. The normal path never constructs dense doubles and remains
    ...
    def method_metadata(self) -> Dict[str, Any]:
        ...
        if self.eri_backend == "cd":
            metadata.update({
                ...
                "quartic_residual": "complete-rr-cd",
    ...
    def _fit_thc_projector(self) -> THCProjectorFactors:
        ...
        if self.thc_rank != dimension:
            raise NotImplementedError(
                ...
                f"must equal the full pair dimension {dimension} until the "
                "complete RR residual exposes a shared paper-equation "
                "decomposition"
            )
        for name, rank in (
            ("thc_rank", self.thc_rank),
