...
stream has synchronized.  Batches of 1, 8, and 32 selected columns are then
...
def _snapshot_evidence_valid(source: dict[str, Any]) -> bool:
    """Validate captured v2 snapshot evidence without filesystem access."""
...
def evaluate_gate(record: dict[str, Any]) -> dict[str, Any]:
    ...
    def check(
        ...
        *,
        ...
        if scope not in {"correctness", "performance"}:
            raise ValueError("gate check scope must be correctness or performance")
        checks.append({
            "id": identifier,
            "scope": scope,
            "passed": bool(passed),
    ...
    operation_ledgers_agree = all(
        (timed_operations.get(direction) or {}).get(name) == item
    ...
    check(
        ...
        == "gpu4pyscf.gint-selected-transfer-ledger.v1"
    ...
    check(
        "performance_transfer_release",
        provider.get("performance_eligible") is True
        and transfers.get("complete_for_vhfopt_internal_setup") is True
        and unresolved == [],
        (
            "provider has not completed its _VHFOpt/runtime transfer audit; "
            "correct numerical evidence cannot be reported as performance "
            "eligible"
        ),
        {
    ...
    performance_eligible = correctness_passed and all(
        item["passed"] for item in checks
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if (
        record["status"] == "completed"
        and record["gate_decision"]["correctness_passed"] is True
    ) else 1
