...
def _qualification_lineage(
    *, qualification_sha256: str, release_sha256: str
) -> dict:
    attestation = {
...
def _convert_manifest_to_v3(source: Path) -> None:
    """Add the empty-link v3 proof to the synthetic snapshot fixture."""
    ...
    _, source_file_count = BENCHMARK._source_tree_digest(source)
...
def test_non_dry_run_computes_effective_method_info_before_initial_record(
    monkeypatch, tmp_path: Path
) -> None:
    class ReachedEligibility(RuntimeError):
        pass

    observed: dict[str, object] = {}

    def capture(record, method_info):
        observed["record"] = record
        observed["method_info"] = method_info
        raise ReachedEligibility

    monkeypatch.setattr(BENCHMARK, "_apply_method_eligibility", capture)
    monkeypatch.setattr(
        BENCHMARK, "_apply_source_start_eligibility", lambda record: None
    )
    monkeypatch.setattr(BENCHMARK, "_git_state", lambda: {"test": True})
    monkeypatch.setattr(BENCHMARK, "_topology", lambda: {})
    monkeypatch.setattr(BENCHMARK, "_pcie_info", lambda: {})
    monkeypatch.setattr(BENCHMARK, "_version", lambda name: "test")
    monkeypatch.setattr(BENCHMARK, "_cpu_model", lambda: "test")
    ...
    with np.testing.assert_raises(ReachedEligibility):
        BENCHMARK.run(args)

    assert observed["record"]["status"] == "starting"
    assert observed["method_info"]["rr_ring_kernel"] == "gemm"
...
def test_effective_rr_method_info_tracks_gemm_without_claiming_reference() -> None:
    args = BENCHMARK._parser().parse_args([
        "--case", "water2-tz",
        "--method", "rr_cd",
        "--rr-ring-kernel", "gemm",
    ...
    metadata = BENCHMARK._effective_method_info(args)
    plan = BENCHMARK._plan(args, BENCHMARK.load_cases()[args.case])

    assert metadata["validation_status"] == (
        "direct-cd-compressed-rr-gemm-gate-pending"
    )
    assert metadata["rr_ring_kernel"] == "gemm"
    assert metadata["performance_eligible"] is False
    assert metadata["performance_gates"]["rr_ring_contraction"] == (
        "gemm-kernel-awaiting-a100-numerical-allocation-gate"
    )
    assert all(
        "reference kernel" not in reason
        for reason in metadata["performance_limitations"]
    )
    assert plan["validation_status"] == metadata["validation_status"]
    assert plan["settings"]["rr_ring_kernel"] == "gemm"


def test_thc_methods_reject_rr_gemm_selector() -> None:
    for method in ("thc_cd", "thc_canonical"):
        args = BENCHMARK._parser().parse_args([
            "--case", "water2-tz",
            "--method", method,
            "--rr-ring-kernel", "gemm",
        ...
        with np.testing.assert_raises_regex(
            ValueError, "applies only to RRCCSD"
        ):
            BENCHMARK._validate_run_configuration(args)


def test_config_cannot_inject_unknown_rr_ring_selector(tmp_path: Path) -> None:
    config = tmp_path / "invalid-ring.json"
    config.write_text(
        json.dumps({"rr_ring_kernel": "bogus"}), encoding="utf-8"
    )
    args = BENCHMARK._parser().parse_args([
        "--case", "water2-tz",
        "--method", "rr_cd",
    ...
    with np.testing.assert_raises_regex(
        ValueError, "one of: reference, gemm"
    ):
        BENCHMARK._validate_run_configuration(args)


def test_config_normalizes_rr_ring_selector_case(tmp_path: Path) -> None:
    config = tmp_path / "ring.json"
    config.write_text(
        json.dumps({"rr_ring_kernel": " GEMM "}), encoding="utf-8"
    )
    args = BENCHMARK._parser().parse_args([
        "--case", "water2-tz",
        "--method", "rr_cd",
    ...
    assert BENCHMARK._effective_method_info(args)["rr_ring_kernel"] == "gemm"
...
def test_thc_cd_configuration_requires_exact_explicit_endpoint() -> None:
    ...
    method = BENCHMARK.METHOD_INFO["thc_cd"]
    assert method["performance_eligible"] is False
    assert method["validation_status"] == (
        "direct-cd-r123-complement-full-pair-thc-endpoint"
    )
    assert method["performance_gates"][
        "shared_rr_paper_residual_decomposition"
    ] == "implemented-proof-unfused"
...
def test_v2_snapshot_is_rejected_by_runtime_eligibility(
    monkeypatch, tmp_path: Path
) -> None:
    ...
    try:
        ...
        assert state["stable_during_run"] is False
        assert state["formal_performance_eligible"] is False
        assert record["performance_eligible"] is False
        assert any("requires a v3 source snapshot" in reason
                   for reason in state["snapshot"]["reasons"])
    finally:
        _make_snapshot_writable(source)


def test_v3_snapshot_qualifies_without_inventing_top_level_eligibility(
    monkeypatch, tmp_path: Path
) -> None:
    task, source, _ = _installed_snapshot(tmp_path)
    _convert_manifest_to_v3(source)
    monkeypatch.setattr(BENCHMARK, "REPOSITORY_ROOT", source)
    monkeypatch.setenv("CCSD_TASK_ROOT", str(task))
    monkeypatch.setenv("CCSD_EXPECTED_DEPLOYMENT_PROFILE", "candidate")
    try:
...
def test_candidate_v3_runtime_manifest_rejects_missing_and_tampered_evidence(
    tmp_path: Path,
) -> None:
    task, source, _ = _installed_snapshot(tmp_path)
    _convert_manifest_to_v3(source)
    try:
        ...
        digest, count = BENCHMARK._source_tree_digest(source)
        arguments = {
            "source_tree_sha256": digest,
            "source_file_count": count,
            "task_root": task,
        }
        assert BENCHMARK._runtime_manifest_valid(manifest, **arguments)

        missing = copy.deepcopy(manifest)
        missing["runtime_binaries"].pop("validation_evidence")
        missing["runtime_binaries"].pop("validation_evidence_sha256")
        assert not BENCHMARK._runtime_manifest_valid(missing, **arguments)

        tampered = copy.deepcopy(manifest)
        tampered["runtime_binaries"]["validation_evidence"][
            "sidecar_bytes"
        ] += 1
        assert not BENCHMARK._runtime_manifest_valid(tampered, **arguments)

        wrong_source = copy.deepcopy(manifest)
        runtime = wrong_source["runtime_binaries"]
        runtime["validation_evidence"]["source_sha256"] = "9" * 64
        runtime["validation_evidence_sha256"] = SNAPSHOT._canonical_json_sha256(
            runtime["validation_evidence"]
        )
        assert not BENCHMARK._runtime_manifest_valid(
            wrong_source, **arguments
        )

        wrong_inventory = copy.deepcopy(manifest)
        wrong_inventory["runtime_binaries"]["files"][0]["sha256"] = "0" * 64
        assert not BENCHMARK._runtime_manifest_valid(
            wrong_inventory, **arguments
        )

        wrong_path = copy.deepcopy(manifest)
        runtime = wrong_path["runtime_binaries"]
        runtime["validation_evidence"]["manifest_path"] = (
            f"/other/runtime-bundle-manifests/"
            f"{runtime['validation_evidence']['bundle_id']}.json"
        )
        runtime["validation_evidence_sha256"] = SNAPSHOT._canonical_json_sha256(
            runtime["validation_evidence"]
        )
        assert not BENCHMARK._runtime_manifest_valid(wrong_path, **arguments)
    finally:
        _make_snapshot_writable(source)
...
def test_online_benchmark_rejects_each_runtime_evidence_alias_tamper(
...
def test_online_benchmark_rejects_extra_candidate_runtime_field(
    tmp_path: Path,
) -> None:
    task, source, _ = _installed_snapshot(tmp_path)
    _convert_manifest_to_v3(source)
    try:
        manifest = json.loads((source.parent / "manifest.json").read_text())
        digest, count = BENCHMARK._source_tree_digest(source)
        manifest["runtime_binaries"]["unsealed_note"] = "unexpected"
...
def test_runtime_manifest_keeps_g0_legacy_and_release_b_lineage(
    tmp_path: Path,
) -> None:
    task, source, _ = _installed_snapshot(tmp_path)
    _convert_manifest_to_v3(source)
    try:
        manifest = json.loads((source.parent / "manifest.json").read_text())
        qualification_sha, qualification_count = (
            BENCHMARK._source_tree_digest(source)
        )

        g0 = copy.deepcopy(manifest)
        g0["local_provenance"]["deployment_profile"] = (
            BENCHMARK.G0_DEPLOYMENT_PROFILE
        )
        g0["local_provenance"]["canonical_pristine"] = True
        g0["runtime_binaries"].pop("validation_evidence")
        g0["runtime_binaries"].pop("validation_evidence_sha256")
        assert BENCHMARK._runtime_manifest_valid(g0)
        ...
        assert BENCHMARK._runtime_manifest_valid(
            release,
            source_tree_sha256=release_sha,
            source_file_count=qualification_count + 1,
            task_root=task,
        )

        wrong_count = copy.deepcopy(release)
        assert not BENCHMARK._runtime_manifest_valid(
            wrong_count,
            source_tree_sha256=release_sha,
            source_file_count=qualification_count + 2,
            task_root=task,
        )
    finally:
        _make_snapshot_writable(source)
