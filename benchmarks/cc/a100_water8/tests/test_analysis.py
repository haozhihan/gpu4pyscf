...
from analyze_results import (  # noqa: E402
    ...
    _route_table,
    _timed_snapshot_evidence_reasons,
    analyze_records,
    compatibility,
    write_outputs,
)
from claim_record import validate_claim_record  # noqa: E402
import snapshot_manifest as SNAPSHOT  # noqa: E402
...
def _candidate_runtime_manifest(
    *, source_sha256: str = SOURCE_DIGEST, source_file_count: int = 20,
    task_root: str = "/task",
) -> dict:
    files = [
        {
        ...
        for index, name in enumerate(SNAPSHOT.REQUIRED_RUNTIME_LIBRARIES)
    ]
    inventory = [
        {
            "name": name,
        ...
        for name, entry in zip(
            SNAPSHOT.REQUIRED_RUNTIME_LIBRARIES, files, strict=True
        )
    ]
    ...
    evidence = {
        ...
        "build_verification_path": (
            f"{task_root}/builds/{source_sha256}"
            "-gint-bounded-workspace-v2-sm80/build-verification.json"
        ),
        "library_count": 13,
...
def _promote_source_to_candidate_v3(
    source: dict,
    *,
    source_file_count: int = 20,
    evidence_source_sha256: str | None = None,
    evidence_source_file_count: int | None = None,
) -> dict:
    promoted = copy.deepcopy(source)
    promoted["files_hashed"] = source_file_count
    snapshot = promoted["snapshot"]
    snapshot["expected_task_root"] = "/task"
    manifest = snapshot["manifest"]
    manifest["schema"] = SNAPSHOT.SOURCE_SNAPSHOT_SCHEMA
    manifest["runtime_binaries"] = _candidate_runtime_manifest(
        source_sha256=(evidence_source_sha256 or promoted["tree_sha256"]),
        source_file_count=(
            source_file_count
            if evidence_source_file_count is None
            else evidence_source_file_count
        ),
    )
    return promoted
...
def test_changed_manifest_hash_is_excluded_from_formal_timing() -> None:
    ...
    assert result["compatible"] is False
    assert any("immutable source snapshot" in reason for reason in result["reasons"])


def test_analyzer_accepts_legacy_v2_and_current_v3_result_records() -> None:
    baseline = _record("canonical", 0, 1000.0)
    candidate = _record("thc_rr", 1, 100.0)

    # Historical records remain analyzable under the explicitly named legacy
    # allowance.
    assert _timed_snapshot_evidence_reasons(baseline) == []
    assert _timed_snapshot_evidence_reasons(candidate) == []

    current = copy.deepcopy(candidate)
    current["source"] = _promote_source_to_candidate_v3(
        current["source"]
    )
    assert _timed_snapshot_evidence_reasons(current) == []
    assert compatibility(baseline, current)["compatible"] is True


def test_candidate_v3_requires_source_bound_runtime_bundle_evidence() -> None:
    source = _promote_source_to_candidate_v3(
        _timed_source_evidence()
    )
    manifest = source["snapshot"]["manifest"]

    missing = copy.deepcopy(source)
    runtime = missing["snapshot"]["manifest"]["runtime_binaries"]
    runtime.pop("validation_evidence")
    runtime.pop("validation_evidence_sha256")
    assert any(
        "validation evidence is missing" in reason
        for reason in _timed_snapshot_evidence_reasons({"source": missing})
    )

    tampered = copy.deepcopy(source)
    tampered["snapshot"]["manifest"]["runtime_binaries"][
        "validation_evidence"
    ]["sidecar_bytes"] += 1
    assert any(
        "evidence digest mismatch" in reason
        for reason in _timed_snapshot_evidence_reasons({"source": tampered})
    )

    wrong_source = copy.deepcopy(source)
    runtime = wrong_source["snapshot"]["manifest"]["runtime_binaries"]
    ...
    assert any(
        "source digest mismatch" in reason
        for reason in _timed_snapshot_evidence_reasons({"source": wrong_source})
    )

    wrong_inventory = copy.deepcopy(source)
    wrong_inventory["snapshot"]["manifest"]["runtime_binaries"]["files"][0][
        "sha256"
    ] = "0" * 64
    assert any(
        "bundle_id mismatch" in reason
        for reason in _timed_snapshot_evidence_reasons(
            {"source": wrong_inventory}
        )
    )

    wrong_path = copy.deepcopy(source)
    runtime = wrong_path["snapshot"]["manifest"]["runtime_binaries"]
    runtime["validation_evidence"]["bundle_root"] = (
        f"/other/runtime-bundles/{runtime['validation_evidence']['bundle_id']}"
    )
    runtime["validation_evidence_sha256"] = SNAPSHOT._canonical_json_sha256(
        runtime["validation_evidence"]
    )
    assert any(
        "bundle path is outside task root" in reason
        for reason in _timed_snapshot_evidence_reasons({"source": wrong_path})
    )
    assert manifest["runtime_binaries"]["validation_evidence"][
        "library_count"
    ] == 13
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("bundle_id", "e" * 64, "bundle_id alias mismatch"),
        (
            "inventory_sha256",
            "e" * 64,
            "inventory_sha256 alias mismatch",
        ),
        ("sidecar_sha256", "e" * 64, "sidecar_sha256 alias mismatch"),
        (
            "build_verification_sha256",
            "e" * 64,
            "build_verification_sha256 alias mismatch",
        ),
    ),
)
def test_offline_analyzer_rejects_each_runtime_evidence_alias_tamper(
    field: str, replacement: str, reason_fragment: str
) -> None:
    source = _promote_source_to_candidate_v3(_timed_source_evidence())
    runtime = source["snapshot"]["manifest"]["runtime_binaries"]
    runtime[field] = replacement

    reasons = _timed_snapshot_evidence_reasons({"source": source})

    assert any(reason_fragment in reason for reason in reasons)


def test_offline_analyzer_rejects_extra_candidate_runtime_field() -> None:
    source = _promote_source_to_candidate_v3(_timed_source_evidence())
    runtime = source["snapshot"]["manifest"]["runtime_binaries"]
    runtime["unsealed_note"] = "unexpected"

    reasons = _timed_snapshot_evidence_reasons({"source": source})

    assert any("fields are not exact" in reason for reason in reasons)


def test_g0_v3_retains_explicit_legacy_runtime_compatibility() -> None:
    source = _timed_source_evidence(profile="g0-canonical-pristine")
    source["snapshot"]["manifest"]["schema"] = (
        SNAPSHOT.SOURCE_SNAPSHOT_SCHEMA
    )

    assert _timed_snapshot_evidence_reasons({"source": source}) == []


def test_candidate_release_b_uses_qualification_a_runtime_identity() -> None:
    release_sha = "d" * 64
    source = _promote_source_to_candidate_v3(
        _timed_source_evidence(digest=release_sha),
        source_file_count=21,
        evidence_source_sha256=SOURCE_DIGEST,
        evidence_source_file_count=20,
    )
    manifest = source["snapshot"]["manifest"]
    manifest["gint_release_lineage"] = _release_lineage(
        qualification_sha256=SOURCE_DIGEST,
    ...
    wrong["snapshot"]["manifest"]["gint_release_lineage"][
        "qualification_source_tree_sha256"
    ] = "e" * 64
    assert any(
        "qualification attestation source mismatch" in reason
        or "evidence source digest mismatch" in reason
        for reason in _timed_snapshot_evidence_reasons({"source": wrong})
    )
...
def test_counterpoise_candidate_v3_requires_captured_runtime_evidence() -> None:
    record = _cp_record("canonical", "canonical-signature", -10.0)
    record["source"] = _promote_source_to_candidate_v3(record["source"])
    record["source"]["formal_counterpoise_eligible"] = True
    ...
    runtime = missing["source"]["snapshot"]["manifest"]["runtime_binaries"]
    runtime.pop("validation_evidence")
    runtime.pop("validation_evidence_sha256")
    passed, reasons = _cp_eligibility(missing, SOURCE_DIGEST)
    assert passed is False
    assert any("validation evidence is missing" in reason for reason in reasons)
