...
METHOD_INFO = {
    ...
    "rr_cd": {
        ...
        "performance_gates": {
            "gint_selected_provider_runtime": (
                "reported-separately-from-method-eligibility"
            ),
            "rr_fp64_numerical_gate": "pending-water2-water4",
            "rr_ring_contraction": "bounded-memory-reference-kernel",
        },
        "performance_limitations": [
            "complete RR FP64 water2/water4 numerical gate pending",
    ...
    "thc_cd": {
        ...
        "performance_gates": {
            ...
            "rr_ring_contraction": "not-applicable-to-thc-engine",
        },
        "performance_limitations": [
            "inexact amplitude THC is fail-closed",
            (
                "shared R123/complement proof still evaluates the complete "
                "RR residual and exact R123 separately"
            ),
        ],
    },
    "rr_canonical": {
...
def _normalized_rr_ring_kernel(value: Any) -> str:
    """Normalize and validate the RR execution selector from CLI or config."""
    ...
    if selector not in {"reference", "gemm"}:
        raise ValueError(
            "rr_ring_kernel must be one of: reference, gemm"
        )
    return selector


def _effective_method_info(args: argparse.Namespace) -> dict[str, Any]:
    """Return factual method metadata for the selected execution kernel.

    ``METHOD_INFO`` remains the reference-kernel record used by existing
    protocols.  GEMM is an explicit RR-only validation path until its A100
    numerical and allocation gates have been recorded.  THC has a separate
    residual engine, so describing the RR ring selector as active there would
    be misleading.
    """

    info = METHOD_INFO[args.method]
    selector = _normalized_rr_ring_kernel(
        getattr(args, "rr_ring_kernel", "reference")
    )
    if args.method not in {"rr_cd", "rr_canonical"} or selector == "reference":
        return info
    ...
    updated["rr_ring_kernel"] = "gemm"
    gates = dict(updated.get("performance_gates", {}))
    gates["rr_ring_contraction"] = (
        "gemm-kernel-awaiting-a100-numerical-allocation-gate"
    )
    updated["performance_gates"] = gates
    limitations = [
        reason
        for reason in updated.get("performance_limitations", [])
        if reason != "RR ring contraction is the bounded-memory reference kernel"
    ]
    limitations.append(
        "RR GEMM ring kernel awaits the A100 numerical and allocation gate"
    )
    updated["performance_limitations"] = limitations
    updated["performance_eligible"] = False
    return updated
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "fixed external GINT qualification receipt; selected direct-CD "
    ...
    p.add_argument(
        ...
        default="reference",
        help="exact RR ring implementation; gemm remains performance-gate pending",
    )
...
def _snapshot_at_start(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        evidence["valid"] = False
        evidence.setdefault("reasons", []).append(
            "benchmark runtime requires a v3 source snapshot manifest"
        )
...
def _formal_source_evidence_valid(source: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and _runtime_manifest_valid(
            ...
            source_file_count=source.get("files_hashed"),
...
def _record_source_stability(record: dict[str, Any]) -> bool | None:
    ...
    if not formal:
        ...
        if stable is False:
            ...
            reason = "source is not a validated immutable v3 snapshot"
...
def _apply_source_start_eligibility(record: dict[str, Any]) -> None:
    ...
    if snapshot.get("valid_at_start") is True:
        return
    record["performance_eligible"] = False
    limitation = "source is not a validated immutable v3 snapshot"
    limitations = record.setdefault("limitations", [])
...
def _recorded_gint_runtime_options(
    ...
    *,
    ...
    """Bind a selected-GINT consumer to this recorded snapshot and job.

    The source digest and manifest path come only from the source state captured
    by this benchmark process.  They are never accepted as CLI/config values.
    The provider independently derives its imported source root and rechecks all
    of these values against receipt-pinned snapshot B.
    """
    ...
    if repository != REPOSITORY_ROOT.resolve():
        raise RuntimeError("recorded source state does not identify this benchmark")
    ...
    if recorded_digest != current_digest:
        raise RuntimeError("source changed before selected-GINT construction")
    ...
    if recorded_manifest is not None and Path(recorded_manifest).resolve() != (
        ...
        raise RuntimeError("recorded manifest path is outside this snapshot")
    receipt = args.gint_runtime_gate_receipt
    return {
        "gint_runtime_gate_receipt": (
            None if receipt is None else str(receipt.expanduser().resolve())
        ),
        "gint_runtime_execution_mode": execution_mode,
...
def _rr_cd_constructor_options(
    args: argparse.Namespace,
    *,
    ...
    options = {
        ...
        "rr_ring_kernel": _normalized_rr_ring_kernel(args.rr_ring_kernel),
        "precision": args.precision,
    }
    if runtime_source_state is not None:
        options.update(_recorded_gint_runtime_options(
            args,
...
def _thc_cd_constructor_options(
    args: argparse.Namespace,
    *,
    runtime_source_state: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    return {
        **_rr_cd_constructor_options(
            args, runtime_source_state=runtime_source_state
        ),
...
def _make_solver(
    ...
    *,
    ...
    if args.method == "fno":
        ...
        return wrapper, {**_effective_method_info(args), "fno_wrapper": True}
    ...
    if args.method == "rr_cd":
        solver = RRCCSD(mf, **_rr_cd_constructor_options(
            args, runtime_source_state=runtime_source_state
        ))
        _configure_solver(solver, args)
        solver.frozen = 0
        return solver, _effective_method_info(args)
    if args.method == "thc_cd":
        from gpu4pyscf.cc.thc_rrccsd import THCRRCCSD

        solver = THCRRCCSD(mf, **_thc_cd_constructor_options(
            args, runtime_source_state=runtime_source_state
        ))
        _configure_solver(solver, args)
        solver.frozen = 0
...
def _validate_run_configuration(args: argparse.Namespace) -> None:
    args.rr_ring_kernel = _normalized_rr_ring_kernel(args.rr_ring_kernel)
    if (
        args.method in {"thc_cd", "thc_canonical"}
        and args.rr_ring_kernel != "reference"
    ):
        raise ValueError(
            "--rr-ring-kernel applies only to RRCCSD; the THC residual "
            "engine requires rr_ring_kernel=reference (not applicable)"
        )
    if args.gint_runtime_gate_receipt is not None:
        if (
            args.method not in DIRECT_CD_METHODS
...
def _plan(args: argparse.Namespace, case: dict[str, Any]) -> dict[str, Any]:
    ...
    method_info = _effective_method_info(args)
    plan = {
        "protocol_schema": PROTOCOL_SCHEMA,
        "case": args.case,
        "case_role": case["role"],
        ...
        "solver_lifecycle": {
            ...
            "resident_iterations_policy": method_info.get(
...
def run(args: argparse.Namespace) -> int:
    ...
    _apply_method_eligibility(record, method_info)
    ...
    try:
        ...
        with metrics.phase("post_hf"):
            ...
            with metrics.phase("method_setup"):
                solver, method_meta = _make_solver(
                    args, mf, runtime_source_state=record["source"]
                )
            if args.method == "fno":
                cc_obj = solver.build()
        ...
        if args.method in DIRECT_CD_METHODS:
            record["provider_runtime_gate"] = method_meta.get(
                "provider_runtime_gate",
                {
                    "required": args.gint_column_backend == "selected",
                    "validated": False,
                    "status": "not-recorded",
                    "validation_errors": [
